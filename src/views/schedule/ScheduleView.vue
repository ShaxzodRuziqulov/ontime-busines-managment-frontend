...
const columns = computed(() => [
  ...
  { id: null as string | null, name: 'Tayinlanmagan', color: '#94a3b8' },
])
...
const gridHeight = computed(() => Math.max((closeMinutes.value - openMinutes.value) * PX_PER_MIN, 100) + TOP_PAD)
    ...
    <div v-else class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden relative">
      ...
      <div class="overflow-auto" style="max-height: min(65vh, 620px)">
        <div :style="{ minWidth: `${64 + columns.length * COLUMN_WIDTH}px` }">
          <!-- Header -->
          <div class="flex sticky top-0 z-20 bg-slate-50 border-b border-slate-100">
            <div class="w-16 flex-shrink-0 sticky left-0 z-10 bg-slate-50"></div>
            <div
              v-for="col in columns"
              :key="col.id ?? 'unassigned'"
              class="flex-1 px-3 py-2 border-l border-slate-100 flex items-center gap-2"
          ...
          <div class="flex relative" :style="{ height: `${gridHeight}px` }">
            <div class="w-16 flex-shrink-0 relative border-r border-slate-100 sticky left-0 z-10 bg-white">
              <span
                v-for="slot in timeSlots"
                :key="slot.label"
                class="absolute -translate-y-1/2 text-[11px] text-slate-400 pl-1"
            ...
            <div
              v-for="col in columns"
              :key="col.id ?? 'unassigned'"
              class="flex-1 relative border-l border-slate-100 cursor-pointer hover:bg-slate-50/60 transition-colors"
              :style="{ minWidth: `${COLUMN_WIDTH}px` }"
            ...
            >
              ...
              <button
                ...
                :key="b.id"
                class="absolute left-1 right-1 rounded-lg px-2 py-1 text-left overflow-hidden text-white text-xs shadow-sm hover:brightness-95 transition-all z-10"
                :class="bookingStatusBlockColors[b.status]"
                :style="blockStyle(b)"
                @click="selectedBooking = b"
              >
                <div class="font-medium truncate leading-tight">{{ b.customerName || b.guestName || 'Mijoz' }}</div>
                <div v-if="b.offeredServiceName" class="truncate opacity-90 leading-tight">{{ b.offeredServiceName }}</div>
              </button>
            </div>
            ...
            <div
              ...
              :style="{ top: `${nowTop}px` }"
            >
              <span class="absolute -left-1.5 -top-1.5 w-3 h-3 rounded-full bg-red-500" />
            </div>
          </div>
        </div>
      </div>
