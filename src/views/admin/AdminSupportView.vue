<template>
  <div class="grid w-full min-w-0 max-w-full gap-5 lg:grid-cols-[minmax(280px,.82fr)_minmax(420px,1.18fr)]">
    ...
    <section
        ...
        class="min-h-[calc(100vh-10rem)] min-w-0 max-w-full rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5 dark:border-slate-700 dark:bg-slate-800 lg:min-h-0"
    >
      <div
          v-if="!selected"
      ...
      <template v-else>
        ...
        <div class="my-4 max-h-[calc(100vh-26rem)] min-h-48 space-y-3 overflow-y-auto pr-1 lg:max-h-[390px]">
          <div
              ...
              :key="m.id"
              :class="['max-w-[88%] rounded-2xl p-3 text-sm',
               m.sender === 'OPERATOR'
                ? 'ml-auto bg-primary-50 text-primary-50 dark:bg-primary-800 dark:text-primary-50'
